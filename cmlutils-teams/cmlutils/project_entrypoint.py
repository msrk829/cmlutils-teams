    ...
    try:
        ...
        print(
            "\033[34m\tExported {} Collaborators {}\033[0m".format(
                exported_data.get("total_collaborators"), exported_data.get("collaborator_name_list")
            )
        )
        print(
    ...
    try:
        ...
        print(
            "\033[34m\tImported {} Collaborators {}\033[0m".format(
                import_data.get("total_collaborators"), import_data.get("collaborator_name_list")
            )
        )
        print(
        ...
        if verify:
            ...
            log_filedir = os.path.join(export_output_dir, project_name, "logs")
            _configure_project_command_logging(log_filedir, project_name)
            ...
            try:
                ...
                collaborator_diff, collaborator_config_diff = compare_metadata(
                    imported_collaborator_data,
                    exported_collaborator_data,
                    imported_collaborator_list,
                    exported_collaborator_list,
                    skip_field=["id","name","admin"],
                )
                logging.info("Source      Collaborator list {}".format(exported_collaborator_list))
                logging.info("Destination Collaborator list {}".format(imported_collaborator_list))
                logging.info(
                    "All Collaborator in source project is present at destination project ".format(
                        collaborator_diff
                    )
                    if not collaborator_diff
                    else "Collaborator {} Not Found in source and destination".format(
                        collaborator_diff
                    )
                )
                logging.info(
                    ...
                    else "Difference in application Config {}".format(collaborator_config_diff)
                )
                update_verification_status(
                ...
                result = [export_diff_file_list,import_diff_file_list,proj_diff,
                          proj_config_diff,app_diff,app_config_diff,model_diff,model_config_diff,job_diff, job_config_diff, collaborator_diff, collaborator_config_diff]
...
@click.option(
    "--project_csv",
    "-pc",
    help="List of project name stored in csv file that are already migrated. Make sure the project section is present in import-config.ini and export-config.ini file",
    required=True,
)
def project_verify_cmd(project_csv):
    failed_validation_list = []
    project_list = fetch_project_names_from_csv(project_csv)
    for project_name in project_list:
        ...
        try:
            ...
            try:
                ...
                result = [export_diff_file_list,import_diff_file_list,proj_diff,
                          proj_config_diff,app_diff,app_config_diff,model_diff,
                          model_config_diff,job_diff, job_config_diff,
                ...
                if not migration_status:
                    failed_validation_list.append(project_name)
                write_json_file(file_path=import_file, json_data=validation_data)
