...
#collaborator conversion support
def get_collaborator_name(c: str):
    return c.lower()

def _export_collaborator_convert(c: dict):
    c['username'] = get_collaborator_name(c['username'])
    return c
...
class ProjectExporter(BaseWorkspaceInteractor):
    ...
    def get_project_collaborators(self):
        endpoint = Template(ApiV1Endpoints.PROJECT_COLLABORATOR_LIST.value).substitute(
            username=self.username, project_name=self.project_slug
        )
        response = call_api_v1(
            host=self.host,
    ...
    def _export_project_collaborators(self):
        ...
        collaborators,collaborator_name_list = self.collect_export_collaborators_list()
        self.metrics_data["total_collaborators"] = len(collaborators)
        self.metrics_data["collaborator_name_list"] = sorted(collaborator_name_list)

        logging.info("Exporting project collaborators to path %s", filepath)
        write_json_file(file_path=filepath, json_data=collaborators)
    ...
    def collect_export_collaborators_list(self):
        collaborators_raw = self.get_project_collaborators()
        collaborators = list(map(lambda c:_export_collaborator_convert(c), collaborators_raw))
        collaborator_name_list = list(map(lambda c:c['username'], collaborators))
        if len(collaborators) == 0:
            logging.info("Collaborators are not present in the project %s.", self.project_name)
        else:
            logging.info("Project {} has {} Collaborators".format(self.project_name, len(collaborators)))
        return collaborators, sorted(collaborator_name_list)
    ...
    def collect_export_project_data(self):
        ...
        collaborator_data, collaborator_list = self.collect_export_collaborators_list()
        return (
            proj_data,
            proj_list,
            model_data,
...
class ProjectImporter(BaseWorkspaceInteractor):
    ...
    def create_collaborator_v1(self, proj_id: str, collaborator_metadata) -> str:
        try:
            endpoint = Template(ApiV1Endpoints.CREATE_COLLABORATORS.value).substitute(
                project_name=self.project_name,
                username=collaborator_metadata['username'],
            ...
            response = call_api_v1(
                ...
                method="PUT",
                api_key=self.api_key,
                json_data=collaborator_metadata,
                ca_path=self.ca_path,
            )
        ...
        try:
            ...
            if project_list:
                for project in project_list:
                    ...
                    p = response.json()
                    if p["name"] == project_name and p["owner"]["username"] == self.username:
                        return project["id"]
            return None
        except KeyError as e:
            logging.error(f"Error: {e}")
    ...
    def import_metadata(self, project_id: str):
        ...
        self.create_collaborators(
            project_id=project_id, collaborators_metadata_filepath=collaborators_metadata_filepath
        )
        ...
        self.collect_import_collaborators_list()
    ...
    def collect_imported_project_data(self, project_id: str):
        ...
        collaborator_data, collaborator_list = self.collect_import_collaborators_list()
        return (
            proj_data,
            proj_list,
            model_data,
    ...
    def collect_import_collaborators_list(self):
        collaborator_list = self.get_project_collaborators()
        collaborator_name_list = list(map(lambda c:c["username"], collaborator_list))
        if len(collaborator_list) == 0:
            logging.info("Collaborators are not present in the project %s.", self.project_name)
        else:
            logging.info("Project {} has {} Collaborators".format(self.project_name, len(collaborator_list)))
        self.metrics_data["total_collaborators"] = len(collaborator_name_list)
        self.metrics_data["collaborator_name_list"] = sorted(collaborator_name_list)
        return collaborator_list, sorted(collaborator_name_list)

    def translate_collaborator(self, collaborator: dict):
        collaborator.pop("id")
        collaborator.pop("name")
        return collaborator

    def create_collaborators(self, project_id: str, collaborators_metadata_filepath: str) -> bool:
        try:
            collaborators_metadata_list = read_json_file(collaborators_metadata_filepath)
            for collaborator_raw in collaborators_metadata_list:
                collaborator = self.translate_collaborator(collaborator_raw)
                collaborator_name = collaborator["username"]
                try:
                    logging.info("adding collaborator {} to project {}".format(collaborator_name, self.project_name))
                    self.create_collaborator_v1(proj_id = project_id, collaborator_metadata = collaborator)
                except Exception as e:
                    logging.error("Collaborator {} in Project {} failed to migrate,reason{} ".format(collaborator_name, self.project_name, repr(e)))
                    raise
            return
